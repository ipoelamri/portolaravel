...
@section('content')
    <section class ="bg-gray-300 py-12 h-screen">
        <h1 class ="text-2xl font-semibold text-center">Edit Portofolio {{ strtoupper($portofolio->title) }}</h1>
        <div class="container mx-auto flex justify-center items-center">
            <form action="/admin/{{ $portofolio->id }}" method ="post" enctype="multipart/form-data"
                ...
                <div class="border-2 border-cyan-400 bg-gray-300 rounded-xl py-4 w-5/6 mx-auto pl-6">
                    ...
                    <input type="text" name="title" value="{{ old('title') ?? $portofolio->title }}" id="title"
                ...
                <div class="border-2 border-cyan-400 bg-gray-300 rounded-xl py-4 w-5/6 mx-auto mt-4 pl-6">
                    ...
                    <img src ="{{ \Storage::url($portofolio->image) }}" alt="{{ $portofolio->title }}" class="w-48 h-32">
                </div>
                <div class="border-2 border-cyan-400 bg-gray-300 rounded-xl py-4 w-5/6 mx-auto mt-4 pl-6">
                    <label for="image2" class="font-semibold">Gambar 2:</label>
                    <input type="file" name="image2" id="image2" placeholder="masukan gambar"
                        class="ml-4 bg-transparent rounded-sm w-4/6">
                    <img src ="{{ \Storage::url($portofolio->image2) }}" alt="{{ $portofolio->title }}" class="w-48 h-32">
                ...
                <div class="border-2 border-cyan-400 bg-gray-300 rounded-xl py-4 w-5/6 mx-auto mt-4 pl-6">
                    ...
                    <input type="file" name="image3" id="image3" placeholder="masukan gambar"
                        class="ml-4 bg-transparent rounded-sm w-4/6">
                    <img src ="{{ \Storage::url($portofolio->image3) }}" alt="{{ $portofolio->title }}" class="w-48 h-32">
                </div>
                <div class="border-2 border-cyan-400 bg-gray-300 rounded-xl py-4 w-5/6 mx-auto mt-4 pl-6">
                    <label for="image4" class="font-semibold">Gambar 4:</label>
                    <input type="file" name="image4" id="image4" placeholder="masukan gambar"
                        class="ml-4 bg-transparent rounded-sm w-4/6">
                    <img src ="{{ \Storage::url($portofolio->image4) }}" alt="{{ $portofolio->title }}" class="w-48 h-32">
                </div>
                <div class = "border-2  border-cyan-400 bg-gray-300 rounded-xl py-4 w-5/6 mx-auto mt-4 pl-6">
                    <label for ="use" class =" font-semibold">Menggunakan :</label>
                    <input type ="text" name ="use" value ="{{ old('use') ?? $portofolio->use }}" id="use"
                        placeholder="masukan pemograman" class ="ml-4 bg-transparent rounded-sm w-4/6">
                </div>
                <div class = "border-2  border-cyan-400 bg-gray-300 rounded-xl py-4 w-5/6 mx-auto mt-4 pl-6">
                    <label for ="link" class =" font-semibold">Link :</label>
                    <input type ="text" name ="link" value = "{{ old('link') ?? $portofolio->link }}" id="link"
                        placeholder="masukan link" class ="ml-4 bg-transparent rounded-sm w-4/6">
                </div>
